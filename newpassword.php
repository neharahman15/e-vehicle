...
{
	$result = mysql_query("SELECT * FROM user_login WHERE (email = '$get_empid' AND field_3= '$get_field_3'  AND field_4= '$get_field_4' )");

		if ($result)
		{
			$result = mysql_query("update user_login set password = '$get_password' where email = '$get_empid'");
			$response["success"] = 1;	
			echo json_encode($response);
		} 
		else
